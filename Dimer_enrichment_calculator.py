...
with open(Run_summary,'w') as log:
    ...
    '\tDimer slope\tRsq\tConsensus monomer sequence\tMonomer slope\tRsq\t'+
    'Target percent monomer\tTarget percent dimer\tFold change monomer\t'+
...
for dimers in sorted(glob.glob(dimer_motifs)):
    ...
    for c in np.arange(1,5):
        c = str(c)
        file = path+'/Cycle'+c+'/'+TF+'_'+c+'_monomer_' + dimers + \
            ...
            for i, line in enumerate(readfile):
                ...
                if i == 0:
                    ...
                    if ( Cycle1 == True and len(Fold_change_mon) != int(c) ) or ( Cycle1 == False and len(Fold_change_mon)+1 != int(c) ): 
                        ...
                        notes.append('Monomer motif missing on cycle '+str(c)-1)                                                           
            ...
            for i, line in enumerate(readfile):
                ...
                if i == 0:
                    ...
                    if ( Cycle1 == True and len(Fold_change_dim) != int(c) ) or ( Cycle1 == False and len(Fold_change_dim)+1 != int(c) ): 
                        ...
                        notes.append('Dimer motif missing on cycle '+str(c)-1)                                                         
    ...
    plt.legend([M, D] , [f'Monomer: {round(slope_mon,2)}', f'Dimer: {round(slope_dim,2)}', f'Cooperativity factor: {Cooperative}'])
