...
for c in np.arange(1,5):
    c = str(c)
    file = path+'/Cycle'+c+'/'+TF+'_'+c+'_monomer_homer/knownResults.txt'
...
print('Starting dimer parsing...')         
for c in np.arange(1,5):
    c = str(c)
    file = path+'/Cycle'+c+'/'+TF+'_'+c+'_dimer_homer/knownResults.txt'
...
## Write to log
with open(Run_summary,'a') as log:
    log.write('TF\tDimer site\tCooperativity factor\tConsensus dimer sequence\tDimer slope\tRsq\tConsensus monomer sequence\tMonomer slope\tRsq\tFold change monomer\tFold change dimer\tNotes\n'+
    TF+'\t'+ str(dimer_site)+'\t'+str(Cooperative) + 
